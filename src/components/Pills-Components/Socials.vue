    ...
    <div class="mx-3 border-blue-300 border rounded-lg md:flex-1">
      ...
      <div>
        <div v-for="(socialLink, index) in socialStore.socialLinks" :key="index" class="flex items-center space-x-2 my-4">
          ...
          />
          <button
            @click="socialStore.toggleSocialLink(index)"
            type="button"
            class="bg-gray-200 inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
            role="switch"
            :aria-checked="socialLink.enabled"
            :class="{'bg-blue-200': socialLink.enabled, 'bg-gray-200': !socialLink.enabled}"
          >
            <span
              :class="{'translate-x-5': socialLink.enabled, 'translate-x-0': !socialLink.enabled}"
              aria-hidden="true"
      ...
      <div class="mx-8 my-4">
        <div class="inline-block">
          <div id="imageContainer">
            <button
              v-for="(icon, index) in filteredGalleryIcons"
              :key="index"
              @click="socialStore.createTextField(icon)"
              class="py-1 px-2"
            >
              <img :src="icon.imageSrc" height="25" width="25" class="clickable-image rounded-sm" alt="" />
            </button>
          </div>
        </div>
      </div>
